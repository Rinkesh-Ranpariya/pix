...
const QuotesCategories = () => {
  ...
  const getAllQuotesCategory = (params) => {
    dispatch(getQuotesCategories({ lang: selectedLang.value, ...params })).then(
      (res) => {
  ...
  useEffect(() => {
    dispatch(getLanguages()).then(() => {
      ...
      getAllQuotesCategory({ lang: defaultLan._id });
    });
  }, []);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      if (editCategoryId) {
        dispatch(
          updateQuotesCategory({ id: editCategoryId, payload: formData })
        )
          .then(() => {
        ...
        dispatch(setQuotesCategory(formData))
          .then(() => {
  ...
  const handleToggleConfirmationModal = (type = "no") => {
    if (type === "yes") {
      dispatch(deleteQuotesCategory(deleteCategoryId))
        .then(() => {
  ...
  const changeLanguageHandler = (_, language) => {
    ...
    getAllQuotesCategory({ lang: language?.value });
  };
  ...
  const rearrangeHandler = () => {
    ...
    dispatch(rearrangeQuotesCategory(payload)).then((res) => {
