...
import {
  deleteWallpaperCategory,
  getWallpaperCategories,
  rearrangeWallpaperCategory,
...
const WallpaperCategories = () => {
  ...
  const getAllWallpaperCategory = (params) => {
    dispatch(
      getWallpaperCategories({ lang: selectedLang.value, ...params })
    ).then((res) => {
      console.log(res, "res in wallpaper category");
  ...
  useEffect(() => {
    dispatch(getLanguages()).then(() => {
      ...
      getAllWallpaperCategory({ lang: defaultLan._id });
    });
  }, []);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { resetForm }) => {
      ...
      if (editCategoryId) {
        dispatch(
          updateWallpaperCategory({ id: editCategoryId, payload: formData })
        )
          .then(() => {
            toastHandler("Category edited successfully.", "success");
            getAllWallpaperCategory();
          })
          .catch((error) => {
            console.log(error);
        ...
        dispatch(setWallpaperCategory(formData))
          .then(() => {
  ...
  const handleToggleConfirmationModal = (type = "no") => {
    if (type === "yes") {
      dispatch(deleteWallpaperCategory(deleteCategoryId))
        .then(() => {
  ...
  const changeLanguageHandler = (_, language) => {
    ...
    getAllWallpaperCategory({ lang: language?.value });
  };
  ...
  const rearrangeHandler = () => {
    ...
    console.log(payload, "payload in rearrange");
    dispatch(rearrangeWallpaperCategory(payload)).then((res) => {
