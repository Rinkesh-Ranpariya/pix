...
const PopularLiveWallpapers = () => {
  const dispatch = useDispatch();
  const { liveWallpapers, loading } = useSelector(
    (state) => state.liveWallpapers.liveWallpapersTab
  );
  ...
  useEffect(() => {
    dispatch(getLanguages()).then((res) => {
      if (res?.payload?.length) {
        ...
        getAllLiveWallpapers({ lang: defaultLan?._id });
...
export default PopularLiveWallpapers;
