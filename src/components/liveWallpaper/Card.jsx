...
const Card = ({ index, data, fetchFunction, selectHandler, checked }) => {
  ...
  const [deleteLiveWallpaperId, setDeleteLiveWallpaperId] = useState(null);
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      dispatch(editLiveWallpaper({ id: data?._id, payload: formData })).then(
        () => {
          setIsEditable(false);
          toastHandler("Live Wallpaper updated successfully.", "success");
          fetchFunction();
        }
      );
    },
  ...
  const publishLiveWallpaperHandler = () => {
    dispatch(
      publishLiveWallpaper({ language: data.language, id: [data?._id] })
    ).then(() => {
  ...
  const handleDeleteLiveWallpaper = () => {
    setDeleteLiveWallpaperId(data._id);
          ...
          <Grid item xs={12} sm={6} md={3}>
            <div className="flex">
              ...
              <div className="image-container">
                <img src={data?.thumbUrl || Barbi} alt="" />
              </div>
            </div>
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
          ...
          <Grid item xs={12} sm={6} md={1}>
            <div className="flex flex-col">
              ...
              {(userInfo?.isAdmin ||
                (!data?.isPublished && userInfo?.name === data.user)) && (
                <ButtonComponent
                  onClick={handleDeleteLiveWallpaper}
                  btnClass={"btn-danger"}
                  btnText={"Delete"}
                />
              )}
