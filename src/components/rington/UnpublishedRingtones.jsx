...
const UnpublishedRingtons = ({ selectedLanguage }) => {
  ...
  const publishRingtonHandler = () => {
    dispatch(
      publishRington({ language: selectedLang.value, id: selectedRingtons })
    ).then(() => {
      toastHandler("Rington published successfully.", "success");
      getAllRingtons();
    });
  };
  ...
  const handleToggleConfirmationModal = (type = "no") => {
    if (type === "yes") {
      dispatch(
        ...
        .then(() => {
          toastHandler("Rington deleted successfully.", "success");
          getAllRingtons();
        })
        .catch(() => {
  ...
  useEffect(() => {
    dispatch(getLanguages()).then((res) => {
      if (selectedLanguage) {
        ...
        getAllRingtons({ lang: defaultLan?._id, page: 1, limit: 10 });
...
export default UnpublishedRingtons;
